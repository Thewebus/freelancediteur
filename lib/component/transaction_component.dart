...
class TransactionComponentState extends State<TransactionComponent> {
  ...
  init() async {
    ...
    dateFormate = DateFormat("yMMMd").format(DateTime.parse(
        widget.transactionListData!.dateCreated!.date.toString()));
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: boxDecorationDefault(
          borderRadius: radius(), color: appStore.appColorPrimaryLightColor),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Image.asset('assets/order_id_icon.png',
                  height: 24,
                  width: 24,
                  fit: BoxFit.fill,
              ...
              4.width,
              Text(
                      widget.transactionListData!.orderKey
                          .validate()
                          .splitAfter('order_'),
                      style: secondaryTextStyle())
                  .expand(),
              Text(widget.transactionListData!.status.capitalizeFirstLetter(),
                  style: boldTextStyle(
                      color: getOrderStatusColor(
                          widget.transactionListData!.status.validate()))),
            ],
          ),
          16.height,
          Row(
            ...
            children: [
              CachedNetworkImage(
                ...
                imageUrl: lineItems!.productImages!.isNotEmpty
                    ? lineItems!.productImages![0].src.validate()
                    : '',
                fit: BoxFit.cover,
              ),
              16.width,
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('${lineItems!.name.validate()}',
                      style: boldTextStyle(), maxLines: 2),
                  4.height,
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Image.asset('assets/transaction_history.png',
                          height: 24,
                          width: 24,
                          fit: BoxFit.fill,
                          color: appStore.iconSecondaryColor),
                      4.width,
                      Text(
                          widget.transactionListData!.transactionId
                              .splitAfter('ch_')
                              .validate(value: "N/A"),
                          style: secondaryTextStyle()),
                    ],
                  ...
                  8.height,
                  priceWidget(
                      price: lineItems!.total.validate(),
