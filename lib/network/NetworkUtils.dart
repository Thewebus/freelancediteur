...
import 'package:freelancediteur/utils/QueryString.dart';
...
class APICall {
  ...
  Future<http.Response> tokenPostMethod(String endPoint,
      {requireToken = false}) async {
    Map<String, String> buildHeaderTokens() {
      ...
      if (appStore.isLoggedIn) {
        header.putIfAbsent(HttpHeaders.authorizationHeader,
            () => 'Bearer ${appStore.token}, id ${appStore.userId}');
      }
    ...
    var client = new http.Client();
    var response =
        await client.post(Uri.parse(url), headers: buildHeaderTokens());
    return response;
  }
