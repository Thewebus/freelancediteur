...
import 'package:freelancediteur/model/LoginResponse.dart';
...
Future tokenValidate() async {
  return responseHandler(await APICall()
      .tokenPostMethod("jwt-auth/v1/token/validate", requireToken: true));
}
...
Future getRegisterUserRestApi(request) async {
  return responseHandler(await APICall().postMethod(
      "iqonic-api/api/v1/woocommerce/customer/registration", request));
}

Future socialLoginApi(request) async {
  return responseHandler(await APICall()
      .postMethod('iqonic-api/api/v1/customer/social_login', request));
}
...
Future updateOrderRestApi(request, orderId) async {
  return await responseHandler(await APICall()
...
Future getSubCategories(parent) async {
  return responseHandler(
      await APICall().getMethod('wc/v3/products/categories?parent=$parent'));
}

Future getStripeClientSecret(request) async {
  return responseHandler(await APICall().postMethod(
      'store/api/v1/woocommerce/get-stripe-client-secret', request,
      requireToken: true));
}

Future addToCartBook(request) async {
  return responseHandler(await APICall().postMethod(
      'iqonic-api/api/v1/cart/add-cart', request,
      requireToken: true));
}

Future getCartBook() async {
  return responseHandler(await APICall()
      .getMethod('iqonic-api/api/v1/cart/get-cart', requireToken: true));
}

Future deletefromCart(request) async {
  return responseHandler(await APICall().postMethod(
      'iqonic-api/api/v1/cart/delete-cart/', request,
      requireToken: true));
}

Future clearCart() async {
  return responseHandler(await APICall()
      .getMethod('iqonic-api/api/v1/cart/clear-cart', requireToken: true));
}
