...
import 'package:freelancediteur/component/author_details_component.dart';
import 'package:freelancediteur/component/author_details_header_component.dart';
import 'package:freelancediteur/main.dart';
import 'package:freelancediteur/model/AuthorListResponse.dart';
import 'package:freelancediteur/model/DashboardResponse.dart';
import 'package:freelancediteur/network/rest_api_call.dart';
import 'package:freelancediteur/utils/SelectedAnimationType.dart';
...
class AuthorDetails extends StatefulWidget {
  ...
  AuthorDetails(this.url, this.fullName,
      {this.authorDetails,
      this.authorDetails1,
      this.isDetail = false,
      this.bgColor});
...
class _AuthorDetailsState extends State<AuthorDetails> {
  ...
  Future getAuthorList() async {
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await getAuthorBookListRestApi(widget.isDetail == true
                ? widget.authorDetails1!.id
                : widget.authorDetails!.id)
            .then(
          (res) async {
            appStore.setLoading(false);
            Iterable mCategory = res;
            mAuthorBookList = mCategory
                .map((model) => DashboardBookInfo.fromJson(model))
                .toList();
            setState(() {});
          },
        ).catchError(
          ...
          return [
            SliverOverlapAbsorber(
              ...
              sliver: SliverAppBar(
                ...
                titleSpacing: 0,
                backgroundColor:
                    innerBoxIsScrolled ? widget.bgColor : widget.bgColor,
                    ...
                    : Column(
                        ...
                        children: [
                          Align(
                            ...
                            child: Text('${widget.authorDetails!.description!}',
                                style: primaryTextStyle(), maxLines: 10),
                          ).paddingOnly(left: 16, right: 16, top: 16).visible(
                              widget.authorDetails!.description!
                                  .validate()
                                  .isNotEmpty),
                          16.height.visible(widget.authorDetails!.description!
                              .validate()
                              .isNotEmpty),
                          16.height,
                          AnimationLimiter(
                            child: ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                Color bgColor = bookBackgroundColor[
                                    index % bookBackgroundColor.length];
