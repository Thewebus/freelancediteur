...
import 'package:freelancediteur/view/BookStoreView.dart';
import 'package:freelancediteur/view/MyLibraryView.dart';
import 'package:freelancediteur/view/SearchView.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  List<Widget> tabs = [
    ...
    appStore.isLoggedIn
        ? MyLibraryView()
        : CategoriesListScreen(isShowBack: false),
    SearchView(),
  ];

  List<Widget> tabs1 = [
    appStore.isLoggedIn
        ? OfflineScreen(isShowBack: false)
        : NoInternetConnection(isCloseApp: true),
    NoDataComponent(),
    NoDataComponent(),
  ];
      ...
      builder: (_) {
        return SafeArea(
          child: Scaffold(
            ...
            body: IndexedStack(
                children: !appStore.isNetworkConnected ? tabs1 : tabs,
                index: currentPage),
            bottomNavigationBar: Observer(
              builder: (_) => BottomNavigationBar(
                ...
                items: [
                  bottomNavigationBarItem("assets/home_icon.png",
                      keyString(context, "title_bookStore")!),
                  bottomNavigationBarItem(
                      "assets/library_icon.png",
                      appStore.isLoggedIn
                          ? keyString(context, "title_myLibrary")!
                          : keyString(context, "lbl_categories")!),
                  bottomNavigationBarItem("assets/search_icon.png",
                      keyString(context, "title_search")!),
                ],
              ),
            ),
          ),
