...
import 'package:freelancediteur/component/cart_component.dart';
...
//  late NavigationController controller;
  ...
  Future<void> getCartItem() async {
    ...
    await getCartBook().then((value) {
      ...
      myCartList.clear();
      myCartList.addAll(
          mCart.map((model) => MyCartResponse.fromJson(model)).toList());
      myCartList.forEach((element) {
        total = total.toString().toDouble().validate() +
            element.price.toString().toDouble();
        setState(() {});
      });
  ...
  processPaymentApi(var mOrderId, BuildContext context) async {
    ...
    checkoutURLRestApi(request).then((res) async {
      ...
      Map? results = await WebViewScreen(res['checkout_url'], "Payment",
              orderId: mOrderId.toString())
          .launch(context);
  ...
  Future deleteOrder(orderId) async {
    ...
    await isNetworkAvailable().then(
      (bool) async {
        if (bool) {
          await deleteOrderRestApi(orderId)
              .then((res) async {})
              .catchError((onError) {
            ErrorViewScreen(
  ...
  void dispose() {
    if (widget.isDescription!) {
      appStore.isDarkModeOn
          ? setStatusBarColor(appStore.isLoading
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            Text(keyString(context, 'lbl_my_cart')!,
        ...
        children: [
          ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                Color bgColor =
                    bookBackgroundColor[index % bookBackgroundColor.length];
                return CartComponent(
                  ...
                  BookDescriptionScreen(myCartList[index].proId.toString(),
                          bgColor: bgColor)
                      .launch(context,
          ...
          Observer(builder: (context) {
            return noDataFound(title: keyString(context, 'lbl_empty_cart')!)
                .center()
                .visible(!appStore.isLoading && myCartList.isEmpty);
          }),
          Observer(builder: (context) {
            ...
            Row(
              children: [
                Text(keyString(context, 'lbl_total')!,
                    ...
                    .expand(),
                priceWidget(
                    currency: getStringAsync(CURRENCY_SYMBOL),
