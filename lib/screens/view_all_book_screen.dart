...
import 'package:freelancediteur/component/all_sub_category_component.dart';
import 'package:freelancediteur/component/view_all_book_component.dart';
import 'package:freelancediteur/model/AllBookListResponse.dart';
import 'package:freelancediteur/model/CategoryModel.dart';
...
import 'package:freelancediteur/utils/Strings.dart';
  ...
  Future fetchSubCategoryData() async {
    ...
    await getSubCategories(widget.categoryId).then((res) {
      ...
      setState(() {
        ...
        mCategoryModel =
            mCategory.map((model) => CategoryModel.fromJson(model)).toList();
      });
    }).catchError((error) {
      if (!mounted) return;
      appStore.setLoading(false);
          ...
          children: [
            SingleChildScrollView(
              child: Column(
                ...
                children: [
                  AllSubCategoryComponent(mCategoryModel: mCategoryModel)
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return AnimationConfiguration.staggeredList(
                            ...
                            child: SelectedAnimationType(
                              scale: 5,
                              curves:
                                  getIntAsync(ANIMATION_TYPE_SELECTION_INDEX) ==
                                          SLIDE_ANIMATION
                                      ? Curves.decelerate
                                      : Curves.fastLinearToSlowEaseIn,
                              child: ViewAllBookComponent(
                                      bookData: mData,
            ...
            Text(keyString(context, "lbl_no_data_found")!,
                    style: boldTextStyle())
                .center()
                .visible(mBookList.isEmpty && !appStore.isLoading),
            (mBookList.isNotEmpty)
                ? Align(
                    alignment: Alignment.bottomCenter,
                    child: viewMoreDataLoader.visible(appStore.isLoading))
                : Center(
                    child:
