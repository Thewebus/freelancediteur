...
import 'package:freelancediteur/screens/PDFScreen.dart';
import 'package:freelancediteur/screens/dashboard_screen.dart';
import 'package:freelancediteur/screens/epub_screen.dart';
...
String reviewConvertDate(date) {
  try {
    return date != null
        ? init.DateFormat(reviewDateFormat).format(DateTime.parse(date))
        : '';
  } catch (e) {
    log(e.toString());
    return '';
...
readFile(context, String? mBookId, String filePath, String? name,
    {isCloseApp = true}) async {
  if (filePath.contains(".pdf")) {
    Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => PDFScreen(mBookId, filePath, name)));
  } else if (filePath.contains(".epub")) {
    EpubScreen(mBookId: mBookId, mBookPath: filePath, mTitle: name)
        .launch(context);
    if (isCloseApp) Navigator.of(context).pop();
  }
}

Widget titleSilverAppBarWidget(BuildContext? context,
    {String? title1, String? title2, bool? isHome}) {
  return Row(
    ...
    children: [
      Container(
        ...
        decoration: BoxDecoration(
            border:
                Border(bottom: BorderSide(color: Colors.green, width: 3.0))),
        child: Text(
          title1!,
          style: isHome!
...
BottomNavigationBarItem bottomNavigationBarItem(
    String iconData, String tabName) {
...
InputDecoration inputDecoration(String? title,
    {Color? borderColor, Icon? prefixIcon}) {
  ...
  return Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        32.height,
        Text(keyString(context, 'lbl_book_not_available')!,
            style:
                boldTextStyle(size: 18, color: appStore.appTextPrimaryColor)),
        8.height,
      ],
    ),
  );
...
Widget bookNotAvailableWidget(BuildContext context,
  ...
  return Column(
    ...
    children: <Widget>[
      ...
      16.height,
      Text(title!,
          style: boldTextStyle(size: 18, color: appStore.appTextPrimaryColor),
...
Widget priceWidget({String? price, String? currency, int? size}) {
  return Row(
    ...
    children: [
      Text(price! + " ",
          style: boldTextStyle(color: PRIMARY_COLOR, size: size ?? 16)),
      Text(currency!,
          style: boldTextStyle(color: PRIMARY_COLOR, size: size ?? 14)),
    ],
  );
}
...
Widget salePriceWidget({String? salePrice, String? regularPrice, int? size}) {
  return Row(
    ...
    children: [
      ...
      Row(
        children: [
          Text('${getStringAsync(CURRENCY_SYMBOL)}',
                  style: boldTextStyle(color: PRIMARY_COLOR, size: size ?? 16))
              .visible(regularPrice!.isNotEmpty),
          Text(
            '$regularPrice',
            style: boldTextStyle(
                color: salePrice.isNotEmpty ? Colors.grey : PRIMARY_COLOR,
                decoration: salePrice.isNotEmpty
