...
Future<bool> checkPermission(widget) async {
  ...
  if (!status.isGranted) {
    log("PERMISSION NON AUTORISEE >>>>>>>>>>>>>>>>>>>> DEMANDE EN COURS > : ${status} ");
    ...
    log("PERMISSION ACCEPTEE AU PREALABLE >>>>>>>>>>>>>>>>>>>> RAS !!!");
    return true;
  }
}
...
Future<String> decryptFile(String filePath) async {
  ...
  if (filePath.contains('.pdf')) {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".pdf", "");
  } else if (filePath.contains('.mp4')) {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".mp4", "");
  } else if (filePath.contains('.mp3')) {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".mp3", "");
  } else if (filePath.contains('.mov')) {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".mov", "");
  } else if (filePath.contains('.webm')) {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".webm", "");
  } else {
    encryptedFileName =
        filePath.split("/").last.toString().replaceAll(".epub", "");
  }
